...
<title>添加任务</title>
<style type="text/css">
@import url('${pageContext.request.contextPath}/styles/home.css');
</style>
<style type="text/css">
@import url('<c:url value="/components/calendar/skins/aqua/theme.css"/>');
</style>
...
<body>
	...
	<form id="myForm1" action="${pageContext.request.contextPath}/JobProcessServlet?jobtype=0&action=add" method="post">
		<table>
			...
			<tr>
				<td nowrap>开始时间：</td>
				<td><input type="text" id="p_startTime" name="p_startTime" size="20"></td>
				<td><img id="calBegin" src="components/calendar/skins/aqua/cal.gif" border="0" alt="开始时间" style="cursor: pointer">（Trigger执行开始时间，必填）</td>
			</tr>
			<tr>
				<td nowrap>结束时间：</td>
				<td><input type="text" id="p_endTime" name="p_endTime" size="20"></td>
				<td><img id="calEnd" src="components/calendar/skins/aqua/cal.gif" border="0" alt="结束时间" style="cursor: pointer">（Trigger执行结束时间，可以不填写）</td>
			</tr>
			<tr>
				<td nowrap>执行次数：</td>
				<td><input " type="text" name="p_repeatCount" size="20"></td>
				<td>次（表示Trigger启动后执行多少次结束，不填写执行一次）</td>
			<tr>
				<td nowrap>执行间隔：</td>
				<td><input " type="text" name="p_repeatInterval" size="20"></td>
	...
	<form id="myForm2" action="${pageContext.request.contextPath}/JobProcessServlet?jobtype=1&action=add" method="post">
		<table>
			...
			<tr>
				...
				<td><input type="text" name="triggerName" size="20"></td>
				<td>（必填）</td>
			</tr>
			<tr>
				<td nowrap>Cron表达式：</td>
				<td><input type="text" name="cronExpression" size="20"></td>
	...
	<form id="myForm3" action="${pageContext.request.contextPath}/JobProcessServlet?jobtype=2&action=add" method="post">
		<table>
			...
			<tr>
				<td nowrap>Trigger 名称：</td>
				<td colspan="3"><input type="text" name="triggerName" size="20">（必填）</td>
			</tr>
			<tr>
				<td nowrap>每</td>
				<td nowrap><input type="text" name="val" size="5"></td>
	...
	<table>
		<tr>
			<th bgcolor="00ff00"><b>Cron Trigger ...</b></th>
		</tr>
		<tr>
			<td>Cron Trigger功能比较强大，Cron表达式写法多种多样，我没有找到一个比较通用的方式去转换所有Cron表达式， 在此只列起一几种比较简单的情况，希望能起到一个抛砖引玉作用，更多具体情况请参加相关文档。如果你有什么 好的方式也请告知，互相学习。</td>
		</tr>
	</table>
